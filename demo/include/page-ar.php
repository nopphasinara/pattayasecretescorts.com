	...
	class Page {
		function genLink() {
			...
			if(!empty($total_record)) {
				...
				echo '<div class="item_count inline">إظهار من '.$show_from.' إلى '.$show_to.' من أصل '.number_format($total_record, 0, '', ',').'</div><!---->	';
				...
				echo '<div class="go_page button inline"><select name="go_page" class="select_page input" data-link="'.$script_link.$order_by_url.'">'.$option.'</select> صفحة '.number_format($total_page, 0, '', ',').'</div><!---->';
