	...
	class Page {
		function genLink() {
			...
			if(!empty($total_record)) {
				...
				echo '<div class="item_count inline">Mostra da '.$show_from.' a '.$show_to.' su un totale di '.number_format($total_record, 0, '', ',').' elementi</div><!---->	';
				...
				echo '<div class="go_page button inline">Pagina <select name="go_page" class="select_page input" data-link="'.$script_link.$order_by_url.'">'.$option.'</select> di '.number_format($total_page, 0, '', ',').'</div><!---->';
