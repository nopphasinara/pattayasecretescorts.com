<div id="e_search">
	<form action="<?php echo $fullUrl.$sysLang.'/profiles.html'; ?>" method="get" id="form_search">
		...
		<div class="form_item">
			...
			--><div class="inline">
				<div class="button_submit one inline"><a href="javascript:;" title="" data-type="submit" data-id="form_search"><strong>ค้นหา</strong></a></div>
			</div>
		</div>
		<div id="advance_option" class="<?php echo $option_hidden; ?>">
			<div class="form_item">
				...
				--><div class="inline">
					<div class="give_space10"><h3><strong>ตัวเลือกขั้นสูง:</strong></h3></div>
					<div class="text_yellow give_space10">* ทั้งหมดอยู่ภายใต้การบริการให้กับสุขอนามัยส่วนบุคคลที่เหมาะสม</div>
							...
							<?php
								...
								if(!$receives) {
									...
									for($i = 0; $i < $total_item; $i++) {
										...
										echo '<div class="give_space5"><input type="checkbox" name="receives[]" value="'.$receives[$i]['id'].'" '.$checked.' /> รับได้: '.$receives[$i]['name_'.$sysLang.''].'</div>';
									}
								}
							?>
						</div><!--
							...
							<?php
								...
								if(!$offers) {
									...
									for($i = 0; $i < $total_item; $i++) {
										...
										echo '<div class="give_space5"><input type="checkbox" name="offers[]" value="'.$offers[$i]['id'].'" '.$checked.' /> เสนอ: '.$offers[$i]['name_'.$sysLang.''].'</div>';
									}
								}
							?>
						</div>
		...
		<div class="button_advance">
			<div class="button_submit two inline"><a href="javascript:;" title="" data-type="link" class="show_anvance"><strong>แสดงตัวเลือกขั้นสูง</strong></a></div>
		</div>
	</form>
</div>
