	...
	class Page {
		function genLink() {
			...
			if(!empty($total_record)) {
				...
				echo '<div class="item_count inline">Es werden '.$show_from.' bis '.$show_to.' von insgesamt '.number_format($total_record, 0, '', ',').' Einträgen angezeigt</div><!---->	';
				...
				echo '<div class="go_page button inline"><select name="go_page" class="select_page input" data-link="'.$script_link.$order_by_url.'">'.$option.'</select> von '.number_format($total_page, 0, '', ',').' Seiten</div><!---->';
