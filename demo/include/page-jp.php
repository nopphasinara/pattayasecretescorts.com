	...
	class Page {
		function genLink() {
			...
			if(!empty($total_record)) {
				...
				echo '<div class="item_count inline">計'.number_format($total_record, 0, '', ',').' 件より'.$show_from.'件目から'.$show_to.'</div><!---->	';
				...
				echo '<div class="go_page button inline">件目を表示。'.number_format($total_page, 0, '', ',').'ページ中<select name="go_page" class="select_page input" data-link="'.$script_link.$order_by_url.'">'.$option.'</select>ページ目</div><!---->';
