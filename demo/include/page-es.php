	...
	class Page {
		function genLink() {
			...
			if(!empty($total_record)) {
				...
				echo '<div class="item_count inline">Mostrando '.$show_from.' de '.$show_to.' de un total de '.number_format($total_record, 0, '', ',').' damas</div><!---->	';
				...
				echo '<div class="go_page button inline"><select name="go_page" class="select_page input" data-link="'.$script_link.$order_by_url.'">'.$option.'</select> de '.number_format($total_page, 0, '', ',').' páginas</div><!---->';
