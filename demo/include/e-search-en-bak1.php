	...
	<form action="<?php echo $fullUrl.$sysLang.'/profiles.html'; ?>" method="get" id="form_search">
		...
		<div id="advance_option" class="<?php echo $option_hidden; ?>">
			<div class="form_item">
				...
				--><div class="inline">
					<div class="give_space10"><h3><strong>Advanced Option:</strong></h3></div>
					<div class="text_yellow give_space10">* All service subject to appropriate personal hygiene</div>
							...
							<?php
								...
								if(!$receives) {
									...
									for($i = 0; $i < $total_item; $i++) {
										...
										echo '<div class="give_space5"><input type="checkbox" name="receives[]" value="'.$receives[$i]['id'].'" '.$checked.' /> Receives: '.$receives[$i]['name_'.$sysLang.''].'</div>';
									}
								}
							?>
						</div><!--
							...
							<?php
								...
								if(!$offers) {
									...
									for($i = 0; $i < $total_item; $i++) {
										...
										echo '<div class="give_space5"><input type="checkbox" name="offers[]" value="'.$offers[$i]['id'].'" '.$checked.' /> Offers: '.$offers[$i]['name_'.$sysLang.''].'</div>';
									}
								}
							?>
						</div>
