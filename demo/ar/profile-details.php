		...
		<div id="e_content">
			...
			<div id="e_profile">
				<div class="details">
					...
					<div class="size" dir="rtl">
						<div class="item">
							<div class="label inline">
								<strong>الطول:</strong>
							</div><!--
							--><div class="info inline">
								<?php if(empty($data['height'])) { echo '-'; } else { echo $data['height']; } ?> سم
							</div>
						</div>
						<div class="item">
							<div class="label inline">
							...
							--><div class="info inline">
								<?php if(empty($data['weight'])) { echo '-'; } else { echo $data['weight']; } ?> كغ
							</div>
						</div>
						<div class="item">
							<div class="label inline">
								<strong>مقياس الحذاء:</strong>
							</div><!--
							--><div class="info inline">
								<?php if(empty($data['shoe_size'])) { echo '-'; } else { echo $data['shoe_size']; } ?> بالمقياس الأوروبي
							</div>
						</div>
					</div>
					<div class="label">
				...
				<div class="photos">
					...
					<div class="body">
						<?php
							...
							$db->close();
							if(!$photo) {
								echo '<div class="text_center text_red"><strong>Photo is not available.</strong></div>';
							} else {
			...
			<!--e_option-->
			<div id="e_option" dir="rtl">
				<h2 class="title text_yellow give_space15 give_border">الخيارات المتقدمة:</h2>
							...
							for($i = 0; $i < $total_item; $i++) {
								...
								echo '<div class="item">
									...
									--><div class="option inline">
										تتلقى: '.$receives[$i]['name_'.$sysLang.''].'
									</div>
								</div>';
							}
						}
			...
			<!--e_review-->
			<div id="e_review" dir="rtl">
				<?php
					$db->connect();
					$review = $db->select(" select * from ".$prefix."reviews where status = 'enable' and profile_id = '".$data['id']."' order by date_time desc ");
					$db->close();
					if(!$review) {
						echo '<h2 class="title text_yellow give_space15 give_border">التعليقات (0)</h2>';
						echo '<div class="text_center"><strong>Be the first one to write a review.</strong></div>';
					} else {
						$total_item = count($review);
						echo '<h2 class="title text_yellow give_space15 give_border">التعليقات ('.number_format($total_item, 0, '', ',').')</h2>';
						for($i = 0; $i < $total_item; $i++) {
							$star = '';
			...
			<!--e_add_review-->
			<div id="e_add_review" dir="rtl">
				<h2 class="title text_yellow give_space15 give_border">أضف تعليقك</h2>
				<form action="<?php echo $fullUrl.$sysLang.'/review-action.html'; ?>" method="post" id="form_review" use-ajax="yes">
					...
					<div class="form_item">
						<div class="label inline">الإسم*</div><!--
						--><div class="inline">
							<input type="text" name="name" value="" class="input" />
						</div>
					</div>
					<div class="form_item">
						<div class="label inline">البريد الإلكتروني*</div><!--
						--><div class="inline">
							<input type="text" name="email" value="" class="input" />
						</div>
					</div>
					<div class="form_item">
						<div class="label inline">الملاحظات*</div><!--
						--><div class="inline">
							<textarea name="text" rows="5" class="input"></textarea>
						</div>
					</div>
					<div class="form_item">
						<div class="label inline">الدرجة</div><!--
						--><div class="inline">
						...
						--><div class="inline" dir="ltr">
							<div class="button_submit inline fix_button small oswald">
								...
								--><div class="body inline">
									<a href="javascript:;" title="إرسال" data-type="submit" data-id="form_review" class="text_white">إرسال</a>
								</div><!--
