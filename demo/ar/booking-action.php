<?php
	...
	if(!$resp->is_valid) {
		$mw->returnError($xss['form_id'], $referer, 'لم يكن دخلت اختبار reCAPTCHA بشكل صحيح. أعود وحاول مرة أخرى.');
	} else {
		if(empty($xss['service']) || (empty($xss['1st_choice']) && empty($xss['1st_alternative']) && empty($xss['2nd_alternative']) && empty($xss['2nd_team'])) || empty($xss['date']) || empty($xss['salutation']) || empty($xss['first_name']) || empty($xss['last_name']) || empty($xss['email']) || empty($xss['hotel']) || empty($xss['phone_number']) || empty($xss['room_number']) || empty($xss['contact'])) {
			$mw->returnError($xss['form_id'], $referer, 'يرجى تعبئة جميع الحقول المطلوبة.');
		}

		$check_email = $mw->checkEmail(array($xss['email']));
		if(!$check_email) {
			$mw->returnError($xss['form_id'], $referer, 'عنوان البريد الإلكتروني غير صالح، يرجى المحاولة مرة أخرى.');
		}
		...
		$db->close();
		if(!$insert) {
			$mw->returnError($xss['form_id'], $referer, 'لا يمكن حفظ السجلات، يرجى المحاولة مرة أخرى.');
		}
		...
		$mail->Subject = 'شكرا لطلب الحجز إلى دن فوكيت المرافقون الشيطان';
		...
		$mail_content = '
			العزيز '.$xss['first_name'].' '.$xss['last_name'].',
			<br />
			<p>شكرا لطلب الحجز إلى دن فوكيت المرافقون الشيطان. ونحن نهدف للرد على جميع رسائل البريد الإلكتروني في أقرب وقت ممكن، ولكن أرجو أن تسمحوا 24 ساعة التي لتلقي أي رد. لطلب الحجز في نفس اليوم، يمكنك الاتصال بنا بين الساعة 01:00 والساعة منتصف الليل التايلاندية على  <strong>'.$sitePhone.'.</strong></p>
			...
			<p><strong style="color: #b71300;">هذا هو الرد الآلي من علبة بريد بدون طيار. يرجى عدم الرد على هذا البريد الإلكتروني لأنها لن تكون وردت.</strong></p>

			<p style="color: #777;">بإخلاص،<br />
			<strong>'.$cf->get('siteName').'</strong></p>
		';
