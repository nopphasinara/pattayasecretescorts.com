<?php
	...
	if(!empty($xss['keyword'])) {
		...
		$keyword = " and (name_".$sysLang." like '%".$xss['keyword']."%') ";
	}
	
	$script_link = $adminUrl.'services.php?form_id='.$xss['form_id'].$keyword_url;
...
<div id="e_content">
	<div class="content_res">
		...
		<div id="e_tool">
			<!--e_search-->
			<div id="e_search">
				<form action="<?php echo $adminUrl.'services.php'; ?>" method="get" id="form_search">
					...
					<div class="form_item">
						...
						--><div class="size2 give_break10 inline">
							<input type="text" name="keyword" placeholder="service name in english" value="<?php echo $xss['keyword']; ?>" class="input" />
						</div><!--
		...
		<div class="data_list">
			<?php
				# Count item
				$db->connect();
				$total_record = $db->select(" select count(id) as num from ".$prefix."services where id != '' ".$keyword." ");
				$db->close();
				if(!$total_record || empty($total_record['0']['num'])) {
					...
					$db->report = true;
					$db->connect();
					$data = $db->select(" select id, name_".$sysLang.", offers, receives, locked, status, date_time, update_time, update_id from ".$prefix."services where id != '' ".$keyword." ".$order_by." limit ".$start_id.", ".$end_id." ");
					$db->close();
					if(!$data) {
						echo $mw->createTableList('header');
						echo '<tr class="list_item"><td colspan="4" class="text_center">No result founds.</td></tr>';
						...
						?>
						<form action="<?php echo $adminUrl.'services-action.php?form_id=form_delete'; ?>" method="post" id="form_delete">
							...
							<!--/e_subtool-->
							<?php
								echo $mw->createTableList('header', array('name_en', 'status', 'date_time'));
								...
								for($i = 0; $i < $total_item; $i++) {
									echo '<tr class="list_item">
										...
										<td class="details">
											<div class="name give_space5">'.$data[$i]['name_'.$sysLang.''].'</div>
											<div class="info give_space5">
												Offers: '.$array_yes_no[$data[$i]['offers']].' | Receives: '.$array_yes_no[$data[$i]['receives']].'
											</div>
											<div class="clear"></div>
											<div class="updated">'.$mw->lastUpdate($data[$i]['update_time'], $data[$i]['update_id']).'</div>
											<div class="button_edit">
												<a href="'.$adminUrl.'services-form.php?mode=edit&id='.$data[$i]['id'].'&referer='.urlencode($absUri['1']).'">Edit</a>
											</div>
										</td>
