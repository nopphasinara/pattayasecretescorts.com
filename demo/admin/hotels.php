<?php
	...
	$script_link = $adminUrl.'hotels.php?form_id='.$xss['form_id'].$keyword_url;
	...
	<div class="content_res">
		...
		<div id="e_tool">
			...
			<div id="e_search">
				...
				<form action="<?php echo $adminUrl.'hotels.php'; ?>" method="get" id="form_search">
		...
		<div class="data_list">
			<?php
				# Count item
				$db->connect();
				$total_record = $db->select(" select count(id) as num from ".$prefix."hotels where id != '' ".$keyword." ");
				$db->close();
				if(!$total_record || empty($total_record['0']['num'])) {
					...
					$db->report = true;
					$db->connect();
					$data = $db->select(" select id, name, phone_number, locked, status, date_time, update_time, update_id from ".$prefix."hotels where id != '' ".$keyword." ".$order_by." limit ".$start_id.", ".$end_id." ");
					$db->close();
					if(!$data) {
						echo $mw->createTableList('header');
						echo '<tr class="list_item"><td colspan="4" class="text_center">No result founds.</td></tr>';
						...
						?>
						<form action="<?php echo $adminUrl.'hotels-action.php'; ?>" method="post" id="form_delete">
									...
									echo '<tr class="list_item">
										...
										<td class="details">
											...
											<div class="info give_space5">
												Phone No. '.$data[$i]['phone_number'].'
											</div>
											<div class="clear"></div>
											<div class="updated">'.$mw->lastUpdate($data[$i]['update_time'], $data[$i]['update_id']).'</div>
											<div class="button_edit">
												<a href="'.$adminUrl.'hotels-form.php?mode=edit&id='.$data[$i]['id'].'&referer='.urlencode($absUri['1']).'">Edit</a>
											</div>
										</td>
