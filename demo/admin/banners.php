<?php
	...
	$script_link = $adminUrl.'banners.php?form_id='.$xss['form_id'].$keyword_url;
	...
	<div class="content_res">
		...
		<div id="e_tool">
			...
			<div id="e_search">
				...
				<form action="<?php echo $adminUrl.'banners.php'; ?>" method="get" id="form_search">
		...
		<div class="data_list">
			<?php
				# Count item
				$db->connect();
				$total_record = $db->select(" select count(id) as num from ".$prefix."banners where id != '' ".$keyword." ");
				$db->close();
				if(!$total_record || empty($total_record['0']['num'])) {
					...
					$db->report = true;
					$db->connect();
					$data = $db->select(" select id, name, file_name, link, banner_type, banner_side, locked, status, date_time, update_time, update_id from ".$prefix."banners where id != '' ".$keyword." ".$order_by." limit ".$start_id.", ".$end_id." ");
					$db->close();
					if(!$data) {
						echo $mw->createTableList('header');
						echo '<tr class="list_item"><td colspan="4" class="text_center">No result founds.</td></tr>';
						...
						?>
						<form action="<?php echo $adminUrl.'banners-action.php'; ?>" method="post" id="form_delete">
							...
							<?php
								...
								for($i = 0; $i < $total_item; $i++) {
									...
									if($data[$i]['banner_type'] == 'image') {
										$banner_image = '<div class="thumbnail">
											<img src="'.$fullUrl.'image-banner/'.$data[$i]['file_name'].'" width="121" alt="" />
										</div>';
									}
										...
										<td class="details">
											...
											<div class="all_info">
												...
												<div class="info give_space5">
													Type: '.$array_banner_type[$data[$i]['banner_type']].' | Side: '.$array_banner_side[$data[$i]['banner_side']].'
												</div>
												<div class="clear"></div>
											...
											<div class="button_edit">
												<a href="'.$adminUrl.'banners-form.php?mode=edit&id='.$data[$i]['id'].'&referer='.urlencode($absUri['1']).'">Edit</a>
											</div>
										</td>
