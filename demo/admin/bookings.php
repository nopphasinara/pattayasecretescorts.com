<?php
	...
	if(!empty($xss['keyword'])) {
		...
		$keyword = " and (refno like '%".$xss['keyword']."%') ";
	}
	...
	$script_link = $adminUrl.'bookings.php?form_id='.$xss['form_id'].$keyword_url.$new_url;
	...
	<div class="content_res">
		...
		<div id="e_tool">
			...
			<div id="e_search">
				...
				<form action="<?php echo $adminUrl.'bookings.php'; ?>" method="get" id="form_search">
					...
					<div class="form_item">
						...
						--><div class="size2 give_break10 inline">
							<input type="text" name="keyword" placeholder="ref no." value="<?php echo $xss['keyword']; ?>" class="input" />
						</div><!--
		...
		<div class="data_list">
			<?php
				# Count item
				$db->connect();
				$total_record = $db->select(" select count(id) as num from ".$prefix."bookings where id != '' ".$keyword.$new." ");
				$db->close();
				if(!$total_record || empty($total_record['0']['num'])) {
					...
					$db->report = true;
					$db->connect();
					$data = $db->select(" select * from ".$prefix."bookings where id != '' ".$keyword.$new." ".$order_by." limit ".$start_id.", ".$end_id." ");
					$db->close();
					if(!$data) {
						echo $mw->createTableList('header');
						echo '<tr class="list_item"><td colspan="4" class="text_center">No result founds.</td></tr>';
						...
						?>
						<form action="<?php echo $adminUrl.'bookings-action.php'; ?>" method="post" id="form_delete">
							...
							<!--/e_subtool-->
							<?php
								echo $mw->createTableList('header', array('first_name', 'status', 'date_time'));
									...
									echo '<tr class="list_item">
										...
										<td class="details">
											<div class="name give_space5">
												'.$data[$i]['first_name'].' '.$data[$i]['last_name'].$new_item.'
											</div>
											...
											<div class="button_edit">
												<a href="'.$adminUrl.'bookings-form.php?mode=edit&id='.$data[$i]['id'].'&referer='.urlencode($absUri['1']).'">Edit</a>
											</div>
										</td>
										<td class="status '.$array_status_contact_color[$data[$i]['status']].'">'.$array_status_contact[$data[$i]['status']].'</td>
