<?php
	...
	$script_link = $adminUrl.'users.php?form_id='.$xss['form_id'].$keyword_url;
	...
	<div class="content_res">
		...
		<div id="e_tool">
			...
			<div id="e_search">
				...
				<form action="<?php echo $adminUrl.'users.php'; ?>" method="get" id="form_search">
					...
					<div class="form_item">
						...
						--><div class="size2 give_break10 inline">
							<input type="text" name="keyword" placeholder="name or email" value="<?php echo $xss['keyword']; ?>" class="input" />
						</div><!--
		...
		<div class="data_list">
			<?php
				...
				if(!$total_record || empty($total_record['0']['num'])) {
					...
					$db->report = true;
					$db->connect();
					$data = $db->select(" select id, name, email, level, locked, status, date_time, update_time, update_id from ".$prefix."users where id != '' and level != 'system' ".$keyword." ".$order_by." limit ".$start_id.", ".$end_id." ");
					$db->close();
					if(!$data) {
						echo $mw->createTableList('header');
						echo '<tr class="list_item"><td colspan="4" class="text_center">No result founds.</td></tr>';
						...
						?>
						<form action="<?php echo $adminUrl.'users-action.php'; ?>" method="post" id="form_delete">
								...
								for($i = 0; $i < $total_item; $i++) {
									echo '<tr class="list_item">
										...
										<td class="details">
											...
											<div class="info give_space10">
												<div class="email inline give_break10">
													'.$data[$i]['email'].'
												</div><!--
												--><div class="level inline">
													'.$array_level[$data[$i]['level']].'
												</div>
											</div>
											<div class="clear"></div>
											<div class="updated">'.$mw->lastUpdate($data[$i]['update_time'], $data[$i]['update_id']).'</div>
											<div class="button_edit">
												<a href="'.$adminUrl.'users-form.php?mode=edit&id='.$data[$i]['id'].'&referer='.urlencode($absUri['1']).'">Edit</a>
											</div>
										</td>
