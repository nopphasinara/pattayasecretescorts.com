<?php
	...
	$script_link = $adminUrl.'scrolling-texts.php?form_id='.$xss['form_id'].$keyword_url;
	...
	<div class="content_res">
		...
		<div id="e_tool">
			...
			<div id="e_search">
				...
				<form action="<?php echo $adminUrl.'scrolling-texts.php'; ?>" method="get" id="form_search">
		...
		<div class="data_list">
			<?php
				# Count item
				$db->connect();
				$total_record = $db->select(" select count(id) as num from ".$prefix."scrolling_texts where id != '' ".$keyword." ");
				$db->close();
				if(!$total_record || empty($total_record['0']['num'])) {
					...
					$db->report = true;
					$db->connect();
					$data = $db->select(" select id, name_".$sysLang.", link, locked, status, date_time, update_time, update_id from ".$prefix."scrolling_texts where id != '' ".$keyword." ".$order_by." limit ".$start_id.", ".$end_id." ");
					$db->close();
					if(!$data) {
						echo $mw->createTableList('header');
						echo '<tr class="list_item"><td colspan="4" class="text_center">No result founds.</td></tr>';
						...
						?>
						<form action="<?php echo $adminUrl.'scrolling-texts-action.php'; ?>" method="post" id="form_delete">
							...
							<!--/e_subtool-->
							<?php
								echo $mw->createTableList('header', array('name_'.$sysLang.'', 'status', 'date_time'));

								$total_item = count($data);
								for($i = 0; $i < $total_item; $i++) {
									$data[$i]['link'] = (empty($data[$i]['link'])) ? '-' : '<a href="'.$data[$i]['link'].'" target="_blank">'.$data[$i]['link'].'</a>';
									...
									echo '<tr class="list_item">
										...
										<td class="details">
											...
											<div class="button_edit">
												<a href="'.$adminUrl.'scrolling-texts-form.php?mode=edit&id='.$data[$i]['id'].'&referer='.urlencode($absUri['1']).'">Edit</a>
											</div>
										</td>
