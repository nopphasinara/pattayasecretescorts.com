<?php
	...
	if(!$resp->is_valid) {
		$mw->returnError($xss['form_id'], $referer, '该验证码输入不正确。回去再次尝试。');
	} else {
		if(empty($xss['service']) || (empty($xss['1st_choice']) && empty($xss['1st_alternative']) && empty($xss['2nd_alternative']) && empty($xss['2nd_team'])) || empty($xss['date']) || empty($xss['salutation']) || empty($xss['first_name']) || empty($xss['last_name']) || empty($xss['email']) || empty($xss['hotel']) || empty($xss['phone_number']) || empty($xss['room_number']) || empty($xss['contact'])) {
			$mw->returnError($xss['form_id'], $referer, '请填写所有必填字段。');
		}

		$check_email = $mw->checkEmail(array($xss['email']));
		if(!$check_email) {
			$mw->returnError($xss['form_id'], $referer, '电子邮件地址无效，请重试。');
		}
		...
		$db->close();
		if(!$insert) {
			$mw->returnError($xss['form_id'], $referer, '无法保存记录，请重试。');
		}
		...
		$mail->Subject = '感谢您对您的预订请求恶魔之穴普吉岛伴游';
		...
		$mail_content = '
			亲 '.$xss['first_name'].' '.$xss['last_name'].',
			<br />
			<p>感谢您对您的预订请求恶魔之穴普吉伴游。我们的目标是尽快回答所有的电子邮件，但请等待24小时，在其中得到答复。对于当天的预订要求，你可能会在<strong><?php echo $sitePhone; ?></strong>致电我们下午1点到午夜泰国时间之间的时间。</p>
			...
			<p><strong style="color: #b71300;">这是一个无人邮箱的自动回复。因为它不会收到，请不要回复此邮件。</strong></p>

			<p style="color: #777;">真诚的，<br />
			<strong>'.$cf->get('siteName').'</strong></p>
		';
