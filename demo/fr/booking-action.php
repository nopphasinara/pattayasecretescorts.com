<?php
	...
	if(!$resp->is_valid) {
		$mw->returnError($xss['form_id'], $referer, 'Le reCAPTCHA n\'a pas été entré correctement. Revenez en arrière et essayer à nouveau.');
	} else {
		if(empty($xss['service']) || (empty($xss['1st_choice']) && empty($xss['1st_alternative']) && empty($xss['2nd_alternative']) && empty($xss['2nd_team'])) || empty($xss['date']) || empty($xss['salutation']) || empty($xss['first_name']) || empty($xss['last_name']) || empty($xss['email']) || empty($xss['hotel']) || empty($xss['phone_number']) || empty($xss['room_number']) || empty($xss['contact'])) {
			$mw->returnError($xss['form_id'], $referer, 'S\'il-vous-plaît complétez tous les champs requis.');
		}

		$check_email = $mw->checkEmail(array($xss['email']));
		if(!$check_email) {
			$mw->returnError($xss['form_id'], $referer, 'adresse e-mail valide, s\'il vous plaît essayer à nouveau.');
		}
		...
		$db->close();
		if(!$insert) {
			$mw->returnError($xss['form_id'], $referer, 'Vous ne pouvez pas enregistrer dossier, s\'il vous plaît essayer à nouveau.');
		}
		...
		$mail->Subject = 'Nous vous remercions de votre demande de réservation à Den Bangkok Escorts du Diable';
		...
		$mail_content = '
			cher '.$xss['first_name'].' '.$xss['last_name'].',
			<br />
			<p>Nous vous remercions de votre demande de réservation à Den Bangkok Escorts du Diable. Notre objectif est de répondre à tous les e-mails dès que possible, mais s\'il vous plaît permettre à 24 heures pour recevoir une réponse. Pour une même demande de réservation d\'une journée, vous pouvez nous appeler entre les heures de 13 heures et l\'heure de minuit Thai au <strong>'.$sitePhone.'.</strong></p>
			...
			<p><strong style="color: #b71300;">Ceci est une réponse automatique à partir d\'une boîte aux lettres sans pilote. S\'il vous plaît ne pas répondre à cet e-mail car il ne sera pas reçu.</strong></p>

			<p style="color: #777;">Cordialement,<br />
			<strong>'.$cf->get('siteName').'</strong></p>
		';
