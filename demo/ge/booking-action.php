<?php
	...
	if(!$resp->is_valid) {
		$mw->returnError($xss['form_id'], $referer, 'Das reCAPTCHA wurde nicht korrekt eingegeben. Gehen Sie zurück und versuchen Sie es erneut.');
	} else {
		if(empty($xss['service']) || (empty($xss['1st_choice']) && empty($xss['1st_alternative']) && empty($xss['2nd_alternative']) && empty($xss['2nd_team'])) || empty($xss['date']) || empty($xss['salutation']) || empty($xss['first_name']) || empty($xss['last_name']) || empty($xss['email']) || empty($xss['hotel']) || empty($xss['phone_number']) || empty($xss['room_number']) || empty($xss['contact'])) {
			$mw->returnError($xss['form_id'], $referer, 'Bitte füllen Sie alle Felder aus.');
		}

		$check_email = $mw->checkEmail(array($xss['email']));
		if(!$check_email) {
			$mw->returnError($xss['form_id'], $referer, 'Ungültige E-Mail-Adresse, bitte versuchen Sie es erneut.');
		}
		...
		$db->close();
		if(!$insert) {
			$mw->returnError($xss['form_id'], $referer, 'Datensatz kann nicht gespeichert werden, bitte versuchen Sie es erneut.');
		}
