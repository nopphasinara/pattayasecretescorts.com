<?php
	...
	if(!$resp->is_valid) {
		$mw->returnError($xss['form_id'], $referer, 'El reCAPTCHA no se ha introducido correctamente. Volver y probar otra vez.');
	} else {
		if(empty($xss['service']) || (empty($xss['1st_choice']) && empty($xss['1st_alternative']) && empty($xss['2nd_alternative']) && empty($xss['2nd_team'])) || empty($xss['date']) || empty($xss['salutation']) || empty($xss['first_name']) || empty($xss['last_name']) || empty($xss['email']) || empty($xss['hotel']) || empty($xss['phone_number']) || empty($xss['room_number']) || empty($xss['contact'])) {
			$mw->returnError($xss['form_id'], $referer, 'Por favor completa todos los campos requeridos.');
		}

		$check_email = $mw->checkEmail(array($xss['email']));
		if(!$check_email) {
			$mw->returnError($xss['form_id'], $referer, 'dirección de email no es válida, por favor, inténtelo de nuevo.');
		}
		...
		$db->close();
		if(!$insert) {
			$mw->returnError($xss['form_id'], $referer, 'No se puede guardar el registro, por favor, inténtelo de nuevo.');
		}
		...
		$mail->Subject = 'Gracias por su solicitud de reserva a Bangkok Den Escoltas del Diablo';
		...
		$mail_content = '
			querido '.$xss['first_name'].' '.$xss['last_name'].',
			<br />
			<p>Gracias por su solicitud de reserva a Bangkok Den Escoltas del Diablo usted. Nuestro objetivo es responder todos los correos electrónicos tan pronto como sea posible, pero por favor de 24 horas para recibir una respuesta. Para una misma solicitud de reserva día, puede llamar entre las horas de 13:00 y la hora de medianoche tailandesa en <strong>'.$sitePhone.'.</strong></p>
			...
			<p><strong style="color: #b71300;">Esta es una respuesta automática a partir de un buzón de correo no tripulado. Por favor, no responda a este mensaje, ya que no será recibida.</strong></p>

			<p style="color: #777;">Sinceramente,<br />
			<strong>'.$cf->get('siteName').'</strong></p>
		';
