...
<div id="e_wrapper">
	<div class="content_res expand-width">
		...
		<div id="e_content">
			...
			<!--e_option-->
			<div id="e_option">
				<h2 class="title text_yellow give_space15 give_border">Advance Options:</h2>
                ...
                $checked_service = $mw->checkService($data['id'], $receives[$i]['id']) == 'checked' ? '<i class="fas fa-check-circle text-success"></i>' : '<i class="fas fa-check-circle text-danger"></i>';
								echo '<div class="item">
									...
									--><div class="option inline">
										'.$checked_service .' Receives: '. $receives[$i]['name_'.$sysLang.''].'
									</div>
								</div>';
							}
						}
                ...
                $checked_service = $mw->checkService($data['id'], $receives[$i]['id']) == 'checked' ? '<i class="fas fa-check-circle text-success"></i>' : '<i class="fas fa-check-circle text-danger"></i>';
								echo '<div class="item">
									...
									--><div class="option inline">
										'.$checked_service . ' Offers: ' . $offers[$i]['name_'.$sysLang.''].'
									</div>
								</div>';
							}
						}
			...
			<div id="e_review">
				<?php
					...
					$db->close();
					if(!$review) {
						echo '<h2 class="title text_yellow give_space15 give_border">Reviews (0)</h2>';
						echo '<div class="text_center"><strong>Be the first one to write a review.</strong></div>';
					} else {
						$total_item = count($review);
						echo '<h2 class="title text_yellow give_space15 give_border">Reviews ('.number_format($total_item, 0, '', ',').')</h2>';
						for($i = 0; $i < $total_item; $i++) {
							$star = '';
			...
			<!--e_add_review-->
			<div id="e_add_review">
				<h2 class="title text_yellow give_space15 give_border">Add Reviews</h2>
						...
						--><div class="inline">
							<div class="button_submit inline fix_button small oswald">
								...
								--><div class="body inline">
									<a href="javascript:;" title="Submit" data-type="submit" data-id="form_review" class="text_white">SUBMIT</a>
								</div><!--
