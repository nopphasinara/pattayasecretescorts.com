...
<div id="e_wrapper">
	<div class="content_res expand-width">
		...
		<!--e_content-->
		<div id="e_content">
			<h1 class="text-primary text-uppercase mb-4"><strong>Search Our Gallery</strong></h1>
			...
			<?php
				...
				if(!$total_record || empty($total_record['0']['num'])) {
					...
					if(!$data) {
						...
						?>
						<div class="d-none page_box text_right">
							<?php echo $pg->genLink(); ?>
						</div>
						<div>&nbsp;</div>
            <div class="container">
              <?php /*<div class="col-xs-6 col-md-<?php echo (12 / $perrow); ?>">
                <div class="thumbnail">
                  <a href="<?php echo $profile_link; ?>" title="<?php echo $data[$i]['nickname']; ?>">
                    <img src="<?php echo $fullUrl.'image-profile/thumbnail/'.$mw->getThumbnail($data[$i]['id']); ?>" width="100%" alt="<?php echo $data[$i]['nickname']; ?>" />
                  </a>
                </div>
                <div class="name oswald text-center text-uppercase text-lg">
                  <a href="<?php echo $profile_link; ?>" title="<?php echo $data[$i]['nickname']; ?>"><?php echo $data[$i]['nickname']; ?></a>
                </div>
                <div class="d-none label">
                  <?php echo $new_label.$hot_label; ?>
                </div>
              </div>*/ ?>
                ...
                <div class="row">
                  <?php
                    ...
                    for($i = 0; $i < $total_item; $i++) {
                      ...
                      $profile_link = $fullUrl . $sysLang . '/profile-details/' . $data[$i]['id'].'/'.$mw->mod($data[$i]['nickname']).'.html';
                      ?>
                      <div class="profile col-12 col-sm-6 col-md-<?php echo 12 / $perrow; ?> text-center mb-5">
                        <div class="thumbnail">
                          <a href="<?php echo $profile_link; ?>" title="<?php echo $data[$i]['nickname']; ?>">
                            <img class="" src="<?php echo $fullUrl.'image-profile/thumbnail/'.$mw->getThumbnail($data[$i]['id']); ?>" alt="<?php echo $data[$i]['nickname']; ?>" style="width: 100% !important; height: auto !important; overflow: hidden !important;" />
                          </a>
                          <div class="box-slide">
                            <div>Height: <?php echo $data[$i]['height']; ?>cm</div>
                            <div>Weight: <?php echo $data[$i]['weight']; ?>kg</div>
                            <div>Shoe Size: <?php echo $data[$i]['shoe_size']; ?></div>
                          </div>
                        </div>
                        <h5 class="oswald mt-2 text-uppercase"><a href="<?php echo $profile_link; ?>" title="<?php echo $data[$i]['nickname']; ?>"><?php echo $data[$i]['nickname']; ?></a></h5>
                      </div>
                      <?php
                      $n = $n + 1;
                      $n = ($n > $perrow) ? 1 : $n;
